---
title: "Nightingale NMR Metabolite Data QC Report"
author: "L.J.Corbin"
date: "31/03/2022"
---
...
pfile = read.table("U:/Scripts/02.0_nmr_qc_param_file.R", sep = "=",as.is=T)
...
imputed_flag <- as.character( pfile[5,2] )
output_dir <- as.character( pfile[6,2] )
...
orig_data <- readRDS(paste0(processseddata_dir,"nmr_raw_data.rds"))
orig_feature <- readRDS(paste0(processseddata_dir,"nmr_feature_metadata.rds"))
orig_sample <- readRDS(paste0(processseddata_dir,"nmr_sample_metadata.rds"))

## set negative values to missing
orig_data <- orig_data %>% 
    mutate_all(funs(replace(., .<0, NA)))
row.names(orig_data) <- row.names(orig_feature)

## identify derived and raw measures
percent_measures <- orig_feature[grep("%",orig_feature$shortname),"shortname"]
ratio_measures <- orig_feature[grep("/",orig_feature$shortname),"shortname"]
derived_measures <- c(percent_measures, ratio_measures)
raw_measures <- orig_feature[!orig_feature$shortname %in% derived_measures,"shortname"]
...
Number of samples in OrigScale file: `r nrow(orig_sample)`  
  ...
  Number of raw measures: `r length(raw_measures)`  
  Number of derived measures: `r length(derived_measures)`
...
By sample missingness is calculated after excluding derived measures (i.e. percentages and ratios)

```{r missingness, echo=F, quote=F, comment=NA, fig.width=10, fig.height=5 }

# summarise missingness across samples and features
sample_missing <- sample.missing(orig_data[raw_measures,])
...
raw_features_no_missing <- features_no_missing[features_no_missing %in% raw_measures]
...
sample_missing_dat$sampleid <- row.names(sample_missing_dat)
orig_sample$missingness_prefilter <- sample_missing_dat$sample_missing[match(orig_sample$sampleid,sample_missing_dat$sampleid)]

# add summary stats to feature file
feature_missing_dat <- as.data.frame(feature_missing)
feature_missing_dat$shortname <- row.names(feature_missing_dat)
orig_feature$missingness_prefilter <- feature_missing_dat$feature_missing[match(orig_feature$shortname,feature_missing_dat$shortname)]

```

There are `r length(features_no_missing)` features with no data missing.  
  Of these, `r length(raw_features_no_missing)` are raw features (not derived).
...
sd_outlier_summary <- outlier.summary(dtst=orig_data, scaling_flag=0, pdf_filename=sd_outlier_filename, nsd=5, cut=0.01, method="sd")
...
feature_sd_outlier_dat$shortname <- row.names(feature_sd_outlier_dat)
orig_feature$sd_outlier_prefilter <- feature_sd_outlier_dat$feature_sd_outlier_summary[match(orig_feature$shortname,feature_sd_outlier_dat$shortname)]
...
#run outlier function
pc_outlier_summary <- outlier.summary(dtst=orig_data, scaling_flag=0, pdf_filename=pc_outlier_filename, nsd=5, cut=0.01, method="percentile")
...
feature_pc_outlier_dat$shortname <- row.names(feature_pc_outlier_dat)
orig_feature$pc_outlier_prefilter <- feature_pc_outlier_dat$feature_pc_outlier_summary[match(orig_feature$shortname,feature_pc_outlier_dat$shortname)]
...
Samples excluded if more than 20% of features are missing.  
  By sample missingness is calculated after excluding derived measures (i.e. percentages and ratios).  
  Features excluded if they are missing in more than 20% samples.
...
samples_to_exclude <- sample.missing.exclusions(orig_data[raw_measures,], threshold = 0.2) 
...
features_to_exclude <- feature.missing.exclusions(orig_data_clean_1, threshold = 0.2) 
...
orig_sample[which(orig_sample$sampleid %in% samples_to_exclude),c("high_missingness_flag")] <- 1

## add flags to feature metadata
orig_feature$high_missingness_flag <- 0
orig_feature[which(orig_feature$shortname %in% features_to_exclude),c("high_missingness_flag")] <- 1

```

Number of sample exclusions based on >20% missingness: `r length(samples_to_exclude)`  
  Number of feature exclusions based on >20% missingness: `r length(features_to_exclude)`

```{r missingness_post_apply, echo=F, quote=F, comment=NA, fig.width=10, fig.height=5 }

# summarise missingness across samples and features
measures_for_missingness <- raw_measures[which(raw_measures %in% row.names(orig_data_clean))]
sample_missing <- sample.missing(orig_data_clean[measures_for_missingness,])
a = summary(sample_missing)
...
## Total metabolite levels (by sample) (based on filtered data)

Calculated after excluding derived measures (i.e. percentages and ratios).

```{r tpa, echo=F, quote=F, comment=NA, fig.width=10, fig.height=5 }

# calculate tpa
measures_for_tpa <- raw_features_no_missing[which(raw_features_no_missing %in% row.names(orig_data_clean))]
sample_tpa <- sample.tpa(orig_data_clean[measures_for_tpa,])
...
# generate histogram
hist(sample_tpa, main="OrigScale - Distribution of total metabolite levels (across samples)", xlab="Sample total metabolite level")
...
## add flags to sample metadata
orig_sample$tml_3SD_flag <- 0
orig_sample[which(orig_sample$sampleid %in% samples_to_exclude_on_tpa_3),c("tml_3SD_flag")] <- 1

orig_sample$tml_5SD_flag <- 0
orig_sample[which(orig_sample$sampleid %in% samples_to_exclude_on_tpa_5),c("tml_5SD_flag")] <- 1

  
```

Number of sample exclusions based on total metabolite level (+/- 3sd from the mean): `r length(samples_to_exclude_on_tpa_3)`  
  Number of sample exclusions based on total metabolite level (+/- 5sd from the mean) (current criteria): `r length(samples_to_exclude_on_tpa_5)`  
...
Tree generated after excluding derived measures (i.e. percentages and ratios).
...
# make tree
#SpearTree <- make.tree(dtst=orig_data_clean[raw_features_no_missing,],cor_method="spearman",hclust_method="complete")
SpearTree <- make.tree(dtst=orig_data_clean[which(row.names(orig_data_clean) %in% raw_measures),] ,cor_method="spearman", hclust_method="complete")
...
Total number of raw features: `r length(k)`  
  Number of independent features (based on a tree cut height of 0.20): `r length(unique(k))`  

### Generate PCs

PCA run on unique features from tree with complete data (i.e. no missing data points).
...
orig_sample[which(orig_sample$sampleid %in% exclude_round1_id),c("pca1_flag")] <- 1

orig_sample$pca2_flag <- 0
orig_sample[which(orig_sample$sampleid %in% exclude_round2_id),c("pca2_flag")] <- 1
...
plasma <- orig_sample$sampleid[which(orig_sample$`Plasma sample` == "X")]
pyruvate <- orig_sample$sampleid[which(orig_sample$`High pyruvate` == "X")]
lactate <- orig_sample$sampleid[which(orig_sample$`High lactate` == "X")]
glutamine <- orig_sample$sampleid[which(orig_sample$`Low glutamine/high glutamate` == "X")]
extra_fails <- unique(c(plasma,pyruvate,lactate,glutamine))
samples_to_keep <- samples_to_keep[!samples_to_keep %in% extra_fails]
  
## add flags to sample metadata
orig_sample$nmr_quality_flag <- 0
orig_sample[which(orig_sample$sample.name %in% extra_fails),c("nmr_quality_flag")] <- 1

# extract from data files
orig_data_out <- orig_data[features_to_keep,samples_to_keep]
saveRDS(orig_data_out, file = paste0(paste0(processseddata_dir,"nmr_cleaned_data.rds")))

# extract from sample metadata
orig_sample_out <- orig_sample[orig_sample$sampleid %in% samples_to_keep,]
saveRDS(orig_sample_out, file = paste0(paste0(processseddata_dir,"nmr_cleaned_sample_metadata.rds")))

# extract from feature metadata
orig_feature_out <- orig_feature[orig_feature$shortname %in% features_to_keep,]
saveRDS(orig_feature_out, file = paste0(paste0(processseddata_dir,"nmr_cleaned_feature_metadata.rds")))


```

No. of samples excluded due to failing Nightingale QC: `r length(extra_fails)`

## Data overview post QC

The QC'd data files contain `r length(samples_to_keep)` samples and `r length(features_to_keep)` features.  

## Check distributions (based on cleaned data)
...
for (i in 1:9){
  ...
  hist(temp, main=paste0("CompID: ",least_normal[i]), cex.main=1.0, xlab="OrigScale")
}
...
for (i in 1:9){
  ...
  hist(temp, main=paste0("CompID: ",most_normal[i]), cex.main=1.0, xlab="OrigScale")
...
normality_test_orig$shortname <- row.names(normality_test_orig)
orig_feature$shapiro_w_postfilter <- normality_test_orig$shapiro_w[match(orig_feature$shortname,feature_missing_dat$shortname)]
orig_feature$shapiro_p_postfilter <- normality_test_orig$shapiro_p[match(orig_feature$shortname,feature_missing_dat$shortname)]
...
# summarise missingness across samples and features
features_to_use <- raw_measures[which(raw_measures %in% row.names(orig_data_out))]
...
sample_missing_dat <- as.data.frame(sample_missing)
sample_missing_dat$sampleid <- row.names(sample_missing_dat)
orig_sample$missingness_postQC <- sample_missing_dat$sample_missing[match(orig_sample$sampleid,sample_missing_dat$sampleid)]

# add summary stats to feature file
feature_missing_dat <- as.data.frame(feature_missing)
feature_missing_dat$shortname <- row.names(feature_missing_dat)
orig_feature$missingness_postQC <- feature_missing_dat$feature_missing[match(orig_feature$shortname,feature_missing_dat$shortname)]
...
#run outlier function
sd_outlier_summary <- outlier.summary(dtst=orig_data_out, scaling_flag=0, pdf_filename=sd_outlier_filename, nsd=5, cut=0.01, method="sd")
...
feature_sd_outlier_dat <- as.data.frame(feature_sd_outlier_summary)
feature_sd_outlier_dat$shortname <- row.names(feature_sd_outlier_dat)
orig_feature$sd_outlier_postQC <- feature_sd_outlier_dat$feature_sd_outlier_summary[match(orig_feature$shortname,feature_sd_outlier_dat$shortname)]
...
#run outlier function
pc_outlier_summary <- outlier.summary(dtst=orig_data_out, scaling_flag=0, pdf_filename=pc_outlier_filename, nsd=5, cut=0.01, method="percentile")
...
feature_pc_outlier_dat <- as.data.frame(feature_pc_outlier_summary)
feature_pc_outlier_dat$shortname <- row.names(feature_pc_outlier_dat)
orig_feature$pc_outlier_postQC <- feature_pc_outlier_dat$feature_pc_outlier_summary[match(orig_feature$shortname,feature_pc_outlier_dat$shortname)]
...
orig_sample[which(orig_sample$sampleid %in% samples_to_keep),c("qc_flag")] <- "pass"

## add pass/fail flag to feature metadata
orig_feature$qc_flag <- "fail"
orig_feature[which(orig_feature$shortname %in% features_to_keep),c("qc_flag")] <- "pass"

saveRDS(orig_sample, file = paste0(paste0(processseddata_dir,"nmr_sample_metadata_with_QCstats.rds")))
saveRDS(orig_feature, file = paste0(paste0(processseddata_dir,"nmr_feature_metadata_with_QCstats.rds")))
